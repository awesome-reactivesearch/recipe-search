...
const FiltersRenderer = ({ loading, value, rawFilterData, setValue }) => {
  // used for displaying all available filters at once
  const [showAllFilters, setShowAllFilters] = useState(false);
  // an array that would have sorted filters' list based on selected/ not-selected
  const [sortedFilters, setSortedFilters] = useState([]);
  const [selectedFilters, setSelectedFilters] = useState([]);
  useEffect(() => {
    //applying logic for having the selected filters always in
    // front of array and rest following them
  ...
  return (
    <div className="filter-container">
      {loading ? (
        ...
        <>
          {sortedFilters //showing only 7 or less filters by default
            .slice(0, Math.max(7, showAllFilters ? sortedFilters.length : 7))
            .map((item) => {
